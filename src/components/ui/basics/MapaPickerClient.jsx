...
async function reverseGeocode(lat, lng) {
  try {
    ...
    return { departamentoRaw, ciudad, direccion, barrio, displayName: data.display_name || "" };
  } catch { return null; }
}
...
function AddressSearchBar({ onSelect, disabled }) {
  ...
  const handleSelect = (item) => {
    ...
    const barrio = addr.suburb || addr.neighbourhood || "";
    const direccion = [calle, barrio].filter(Boolean).join(", ");
    onSelect({ lat: parseFloat(item.lat), lng: parseFloat(item.lon), departamentoRaw, ciudad, direccion, barrio, displayName: item.display_name || "" });
  };
  ...
  return (
    <div className="relative">
      <input
        ...
        className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-white outline-none focus:ring-2 focus:ring-dental-blue/20 focus:border-dental-blue transition-all placeholder:text-slate-400"
      />
...
/**
 * MapaPickerClient — Mapa interactivo con buscador y geocodificación inversa.
 * Este archivo SOLO se carga en el cliente (importado via next/dynamic en MapaPicker.jsx).
 *
 * Props:
 * - latitud, longitud: coordenadas actuales (o null)
 * - onChange: ({ lat, lng, departamentoRaw, ciudad, direccion, barrio }) => void
 * - height: string (CSS height)
 * - disabled: boolean
 * - centerOn: string — cuando cambia, centra el mapa en esa ubicación
 */
export default function MapaPickerClient({
  ...
  const handleLocationSelect = useCallback(async (pos) => {
    ...
    if (geo) {
      ...
      if (onChange) onChange({ lat: pos.lat, lng: pos.lng, departamentoRaw: geo.departamentoRaw, ciudad: geo.ciudad, direccion: geo.direccion, barrio: geo.barrio });
    } else {
