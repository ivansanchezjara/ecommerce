...
export function validatePhone(prefix, number) {
  ...
  if (digits.length < config.min) return `Muy corto para ${config.label} (mín. ${config.min} dígitos).`;
  if (digits.length > config.max) return `Muy largo para ${config.label} (máx. ${config.max} dígitos).`;
  return null;
}
...
export default function PhoneInput({
  ...
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      ...
      {(error || helperText) && (
        <Text variant="bodySm" className={cn("text-xs", error ? "text-red-500" : "text-slate-400")}>
          {error || helperText}
        </Text>
      )}
    </div>
